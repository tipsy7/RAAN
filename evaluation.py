...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, bboxes, depends, lengths, ids,word_freqs) in enumerate(data_loader):
    ...
    for i, (images, captions, bboxes, depends, lengths, ids,word_freqs) in enumerate(data_loader):
        ...
        if img_embs is None:
            ...
            all_word_freqs = np.zeros(
                (len(data_loader.dataset), max_n_word, word_freqs.size(2)))
        ...
        all_word_freqs[ids, :max(lengths), :] = word_freqs.data.cpu().numpy().copy()
    ...
    return img_embs, cap_embs, all_bbx, all_depends, cap_lens,all_word_freqs
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    img_embs, cap_embs, bbox, depends, cap_lens,all_word_freqs = encode_data(
        model, data_loader)
    ...
    if not fold5:
        ...
        sims = shard_xattn(model, img_embs, cap_embs, bbox,
                           depends, cap_lens,all_word_freqs, opt, shard_size=196)
        np.save('sims_flickr_0.020.01*_testfromtrain.npy', sims)
        end = time.time()
        ...
        for i in range(5):
            ...
            sims = shard_xattn(model, img_embs_shard, cap_embs_shard,
                               bbox_shard, depends_shard, cap_lens_shard,all_word_freqs, opt, shard_size=80)
            end = time.time()
            print("calculate similarity time:", end - start)
...
def evalstack(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        start = time.time()
        sims = shard_xattn(model, img_embs, cap_embs, bbox,
                           depends, cap_lens,all_word_freqs, opt, shard_size=80)
        end = time.time()
        print("calculate similarity time:", end - start)
        return sims
        ...
        for i in range(5):
            ...
            start = time.time()
            sims = shard_xattn(model, img_embs_shard, cap_embs_shard,
                               bbox_shard, depend_shard, cap_lens_shard,all_word_freqs, opt, shard_size=80)
            end = time.time()
            print("calculate similarity time:", end - start)
...
def shard_xattn(model, images, captions, bbox, depends, caplens,all_word_freqs, opt, shard_size=128):
    """
    ...
    for i in range(int(n_im_shard)):
        ...
        for j in range(int(n_cap_shard)):
            ...
            wf = Variable(torch.from_numpy(
                all_word_freqs[cap_start:cap_end]), volatile=True).cuda().float()

            dep = depends[cap_start:cap_end]
            # pdb.set_trace()
            try:
                sim,_,_ = model.forward_sim(im, s, bbx, dep, l,wf)
            except Exception:
                pdb.set_trace()
