...
def main():
    ...
    parser.add_argument('--val_step', default=10000000000000, type=int,
...
def validate(opt, val_loader, model):
    ...
    img_embs, cap_embs, bboxes, depends, cap_lens,all_word_freqs = encode_data(
        model, val_loader, opt.log_step, logging.info)

    img_embs = numpy.array([img_embs[i] for i in range(0, len(img_embs), 5)])

    start = time.time()
    sims = shard_xattn(model, img_embs, cap_embs, bboxes, depends, cap_lens,all_word_freqs,
                       opt, shard_size=128)  # changed from 128 to 64
    end = time.time()
