...
class GSMN(object):
    ...
    def __init__(self, opt):
        ...
        self.params = params
        # self.criterion_frequency = torch.nn.SmoothL1Loss()
        self.criterion_frequency = nn.MSELoss()
    ...
    def forward_sim(self, img_emb, cap_emb, bbox, depends, cap_lens,word_freqs):

        i2t_scores,text_few,rare_vector = self.i2t_match_G(
            img_emb, cap_emb, bbox, cap_lens, word_freqs,self.opt)

        t2i_scores = self.t2i_match_G(
            img_emb, cap_emb, depends, cap_lens,rare_vector, self.opt)

        scores = i2t_scores + t2i_scores
        return scores,text_few,text_few
    ...
    def train_emb(self, images, captions, bboxes, depends, lengths, ids=None, word_freqs=None, *args):
        ...
        if img_emb.shape[0]!= cap_emb.shape[0]:
            pdb.set_trace()
        word_freqs = Variable(word_freqs).cuda()

        scores,i2t_text_word_frequency,t2i_text_word_frequency = self.forward_sim(img_emb, cap_emb, bboxes, depends, cap_lens,word_freqs)

        self.optimizer.zero_grad()


        loss_t2i_freq = self.criterion_frequency( t2i_text_word_frequency,word_freqs)
        loss_matching = self.forward_loss(scores)
        loss = loss_matching+300*loss_t2i_freq
